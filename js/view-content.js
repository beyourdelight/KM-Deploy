...
async function loadVideoContent() {
    ...
    try {
        ...
        apiUrl += `&populate[content]=true`;                     // เอา content
...
function processData(item) {
    ...
    // Description: ใช้ ID 'content-body' และรองรับ Rich Text
    const contentBody = document.getElementById('content-body'); 
    
    if (contentBody) {
        if (!itemContent) {
            contentBody.innerHTML = '<p>-</p>'; // ไม่มีข้อมูล
        } else if (typeof itemContent === 'string') {
             // ถ้าเป็น String ธรรมดา (Plain Text)
            contentBody.innerHTML = itemContent.replace(/\n/g, '<br>');
        } else if (Array.isArray(itemContent)) {
             // ถ้าเป็น Rich Text Blocks (จาก Strapi)
            contentBody.innerHTML = renderRichText(itemContent);
        } else {
            // กรณีที่ไม่รู้จักฟอร์แมต
            contentBody.innerHTML = '<p class="text-danger">Error: Unknown description format.</p>';
        }
    }
    // --- 4. Logic ยอดวิว ---
    ...
    if (videoList.length > 0) {
        ...
        if (video.sourceType === 'Direct') {
            ...
            if (video.directFile && video.directFile.url) {
                ...
                // Direct file มักจะต้องมีพื้นหลังดำ (bg-dark) หากวิดีโอโหลดช้า
                playerContainer.classList.add('bg-dark'); 
